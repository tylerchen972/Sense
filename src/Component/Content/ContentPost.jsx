...
export default class ContentPost extends React.Component {
  ...
  deletePost(postID) {
    ...
    fetch(process.env.REACT_APP_API_PATH+"/user-artifacts/"+JSON.parse(this.props.post.content).img_id, {
      method: "DELETE",
      headers: {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer '+sessionStorage.getItem("token")
      ...
      fetch(process.env.REACT_APP_API_PATH+"/user-artifacts/"+JSON.parse(this.props.post.content).audio_id, {
        method: "DELETE",
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer '+sessionStorage.getItem("token")
  ...
  render() {
    ...
    return (
      ...
      {this.showDelete()}
        ...
        <h1 class = "commentText" > {JSON.parse(this.props.post.content).title}</h1>

        <img className={classes.post__image_cover} src={"https://webdev.cse.buffalo.edu" + this.props.post.thumbnailURL}/>

        <audio controls autoplay>
          <source src= {"https://webdev.cse.buffalo.edu" + JSON.parse(this.props.post.content).audio_url} type="audio/mpeg"/>
        </audio>
